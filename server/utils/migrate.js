...
async function migrateAds() {
    try {
      ...
      for (let ad of ads) {
        // Verifica si el anuncio tiene el campo 'adType'
        if (!ad.adType) {
        ...
        try {
          ...
          switch (ad.adType) {
            ...
            case "Servicios":
              ...
              break;
            default:
              console.log(`Tipo de anuncio no reconocido: ${ad.adType}`);
          }
          ...
          // Registrar el error y continuar con el siguiente anuncio
          console.error(`Error migrando el anuncio ${ad._id}:`, migrationError.message);
        }
      }
